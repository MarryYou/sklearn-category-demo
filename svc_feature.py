...
def get_segment():
    ...
    for cur in cursor:
        # print('cutting %s\n'%cur['title'])
        #这里是正则替换一些标点符号和不可见字符
        seg_list = jieba.cut(re.sub('[\s+\.\!\/_,$%^*(+\"\')]+|[+——()?【】“”！，。？、~@#￥%……&*：:-《》（）；]+', '', cur['content']))
        line =''
        for str in seg_list:
            line = line  +' '+str
        print('join success')
        corpus.append({'splitcontent': line, 'oldcontent': cur['content'], 'category':cur['category'],'title':cur['title']})
...
def classify():
    ...
    for index in range(0,len(update_ids)):#这里是把机器学习的自动分类信息重写到数据库中
        ...
        with open('result.txt','a+',encoding='utf-8') as f:
            ...
            f.write(data)
        client.update_one({'_id': update_id}, {'$set': {"category": predicted_category}})
def main():
    # get_segment()
    #这里是分词的函数
    #classify()
